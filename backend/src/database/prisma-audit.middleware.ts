...
export function createAuditMiddleware(
  ...
  return async (params, next) => {
    ...
    // Obt�m refer�ncia ao PrismaClient para salvar o log
    // O log � salvo via SQL direto para evitar loop de middleware
    try {
        ...
        if (client) {
          await client.audit_log.create({
            data: {
              ...
              new_values:
                action !== 'DELETE' ? (params.args?.data ?? null) : null,
      ...
      // N�o deixa falhar a requisi��o principal por causa do log
    }
